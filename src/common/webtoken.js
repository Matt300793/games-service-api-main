...
function create(payload) {
    ...
    const userToken = `${salt}.${payload}.${crypto.createHmac("sha256", config.tokenSecretKey)
                                                  .update(`${salt}.${payload}`)
                                                  .digest("base64")}`;
...
function verify(userToken, dbToken) {
   ...
   const userComputedSignature = crypto.createHmac("sha256", config.tokenSecretKey)
                                       .update(`${salt}.${payload}`)
                                       .digest("base64");
   ...
   const dbComputedToken = crypto.createHmac("sha256", config.tokenSecretKey)
                                 .update(dbToken)
                                 .digest("base64");
